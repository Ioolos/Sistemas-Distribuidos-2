...
public class Servidor {
    ...
    // Lock explícito para la lista de notas
    private static final ReentrantReadWriteLock notasLock = new ReentrantReadWriteLock();
    
    public static void main(String[] args) {
        notasLock.writeLock().lock();
        try {
            notas.add(new Nota(1, "Bienvenida", "Bienvenido al gestor de notas"));
            notas.add(new Nota(2, "Recordatorio", "Revisar correos importantes"));
            notas.add(new Nota(3, "Lista de compras", "Leche, Pan, Huevos"));
        } finally {
            notasLock.writeLock().unlock();
        }
        
        try {
            ServerSocket ss = new ServerSocket(3000);
            System.out.println("Servidor de Gestor de Notas (V2) iniciado en el puerto 3000...");
            System.out.println("Usando sincronización con ReentrantReadWriteLock");
    ...
    private static void crearNota(BufferedReader entrada, PrintWriter salida) {
        try {
            ...
            try {
                int proximoId = 1;
                if (!notas.isEmpty()) {
                    int maxId = notas.stream().mapToInt(Nota::getId).max().orElse(0);
                    proximoId = maxId + 1;
                }
                
                Nota nuevaNota = new Nota(proximoId, titulo, contenido);
                notas.add(nuevaNota);
                salida.println("OK|Nota creada exitosamente con ID: " + nuevaNota.getId());
                System.out.println("Nota creada: " + titulo + " (ID: " + proximoId + ")");
            } finally {
                notasLock.writeLock().unlock();
            }
        } catch (IOException e) {
